...
#    if [[ -z "$serial" && "${drive:0:4}" != "nvme" ]]; then
    ...
    # Log drive num, vendor and model to syno_smart_info.stat
...
log_bad(){ 
    ...
    # $1 for synodisk is "197 Current_Pending_Sector         200    200    000    52"
    local var1
    local var2
    if [[ $drive_type == "scsi" ]]; then  # SAS drive
        if echo "$1" | grep -q -e 'Total.*corrected'; then
            # Get attribute name
        ...
        elif [[ $synodisk == "yes" ]]; then
            # Get attribute name
            #var1=$(echo "$1" | awk '{printf "%-28s", $2}')
            var1=$(echo "$1" | awk '{printf $2}')
            # Get attribute value
            var2=$(echo "$1" | awk '{printf $6}' | awk -F"+" '{print $1}' | cut -d"h" -f1)
        fi
    else
        # Get attribute name
        #var1=$(echo "$1" | awk '{printf "%-28s", $2}')
    ...
    # Don't show "Increased by #" or " Decreased by #" if first time adding the drive to syno_smart_info.stat
...
#echo "debug strin: $strIn"  # debug ############################################
    ...
    if [[ "$strIn" ]]; then
        if [[ $drive_type == "scsi" ]] && [[ $synodisk != "yes" ]]; then  # SAS drive using smartctl
            var1="$3"
            # Get attribute value
            var2=$(echo "$strIn" | awk -F':' '{printf $2}' | xargs)
        else
            ...
            # Get attribute value
            if [[ $synodisk == "yes" ]]; then
                var2=$(echo "$strIn" | awk '{printf $6}' | awk -F"+" '{print $1}' | cut -d"h" -f1)
            else
...
decode_seagate_raw(){ 
    local raw_dec=$1
    if ! [[ $raw_dec =~ ^[0-9]+$ ]]; then
        echo "Usage: decode_seagate_raw <decimal_raw_value>"
        return 1
    fi

    # Pad to 12 hex digits (48 bits)
    local hex12
    hex12=$(printf "%012X" "$raw_dec")

    # Split into bytes (big-endian): b0 b1 b2 b3 b4 b5
    local b0=${hex12:0:2}
    local b1=${hex12:2:2}   # error byte
    local b2=${hex12:4:2}
    local b3=${hex12:6:2}
    local b4=${hex12:8:2}
    local b5=${hex12:10:2}

    # Assemble lower 4 bytes into a 8-hex-digit string (big-endian)
    local ops_hex="${b2}${b3}${b4}${b5}"

    # Enable locale-based thousands separators
    LC_NUMERIC=en_US.UTF-8

    sg_errors=$((0x$b1))
    ops=$((0x$ops_hex))
    sg_ops=$(printf "%'d" "$ops")  # Print with thousands separators
}

synodisk_smart_atts(){ 
    id="" name="" current="" worst=""
    threshold="" raw="" parts=""
    smart_atts=()
    readarray -t array < <(synodisk --smart_info_get /dev/"$1")
    for i in "${array[@]}"; do
        if [[ $i =~ ^Id: ]]; then
            id="$(printf "%3s\n" "${i#Id: }")"
            parts=2
        elif [[ $i =~ ^Name: ]]; then
            name="$(printf '%-30s' "${i#Name: }")"
            parts=1
        elif [[ $i =~ ^Current: ]]; then
            current="$(printf '%-6s' "${i#Current: }")"
            parts=3
        elif [[ $i =~ ^Worst: ]]; then
            worst="$(printf '%-6s' "${i#Worst: }")"
            parts=4
        elif [[ $i =~ ^Threshold: ]]; then
            threshold="$(printf '%-6s' "${i#Threshold: }")"
            parts=5
        elif [[ $i =~ ^Raw: ]]; then
            #raw="$(printf '%-17s' "${i#Raw: }")"
            raw="$(printf '%s' "${i#Raw: }")"
            parts=6
        fi

#echo "debug 1: raw = '$raw'"  # debug ##########################################

        # Convert Seagate attributes 1, 7 and 195
        if [[ $seagate == "yes" ]] && [[ $raw ]]; then
            if [[ $id == "  1" ]] || [[ $id == "  7" ]] || [[ $id == "195" ]]; then
                #decode_seagate_raw "$(echo "$raw" | xargs)"
                decode_seagate_raw "$raw"
                raw="$sg_errors"
            fi
        fi

#echo "debug 2: raw = '$raw'"  # debug ##########################################

        if [[ $parts == 6 ]]; then
            attribute="$id $name $current $worst $threshold $raw"
            smart_atts+=("$attribute")
            parts="0"
        fi

#echo "debug 3: $attribute"  # debug ############################################

    done

    #echo -e "\ndebug smart_atts:"    # debug ####################################
    #for a in "${smart_atts[@]}"; do  # debug ####################################
    #    echo "$a"                    # debug ####################################
    #done                             # debug ####################################
    #echo                             # debug ####################################
...
show_health(){ 
    ...
    if [[ $health_bad == "yes" ]] || [[ $all == "yes" ]]; then
        ...
        synodisk=""
        if ! synodisk --smart_info_get /dev/"$drive" | grep -q "Failed to get the smart info"; then
#            readarray -t smart_atts < <(synodisk --smart_info_get /dev/"$drive")
            synodisk_smart_atts "$drive"
            synodisk="yes"
        elif [[ $seagate == "yes" ]] && [[ $smartversion == 7 ]]; then
        ...
        for strIn in "${smart_atts[@]}"; do
            #if [[ $drive_type == "scsi" ]]; then  # SAS drive
            if [[ $drive_type == "scsi" ]] && [[ $synodisk != "yes" ]]; then  # SAS drive
            ...
            else  # SATA drive or SAS drive with synodisk
                if [[ ${strIn:0:3} == "  1" ]]; then
                    # 1 Raw read error rate
                    if [[ $seagate == "yes" ]]; then
                        if [[ $smartversion == 7 ]] || [[ $synodisk == "yes" ]]; then
                ...
                elif [[ ${strIn:0:3} == "191" ]]; then
                    # 191 G-Sense Error Rate, Shock Sense
                    short_attibutes "191" none
...
# Create syno_smart_info.stat file if needed
smart_log="${logpath}"/syno_smart_info.stat
if [[ ! -f $smart_log ]]; then
    touch "$smart_log"
